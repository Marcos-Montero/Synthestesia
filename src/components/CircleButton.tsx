...
export const CircleButton = ({ note, color, pressed }: Props) => {
	...
	const handleClick = () => {
		...
		play(note)
		setKeyPressed(keys?.find(key => matchKey(key.note, note)))
	}
	useEffect(() => {
		if (!pressed) {
			return
	...
	return (
		<motion.svg
			animate={controls}
			onClick={handleClick}
			className={"h-[40px] w-[40px] hover:cursor-pointer rounded-full border-solid  bg-white/10 backdrop-blur-xl shadow-xl border-b-cyan/2 border-b-2 border-t-zinc/5 border-t-2 "}
		>
			<motion.circle cx="50%" cy="50%" r={20} fill="transparent" />
			<path
